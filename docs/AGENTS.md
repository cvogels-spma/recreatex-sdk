...
| Summing `FindSales` `lines[].totalPrice` directly | **Recreatex returns duplicate line objects** (same `id`, same `sequenceNumber`, same amount; mostly on lines with `mainOrganisedVisitId`). Dedupe by line `id` first. Unfixed, one production month read 284.934,70 € instead of 200.237,10 € — and only reconciled against `paymentLines` after the fix. |
| `Sale.saleDate` is undefined | The field is **`date`**. `saleDate` never existed; the old type declared it as required and it silently read `undefined`. |
| Tracing a bon back to the RFID band it was booked on | Impossible. A `Sale` carries no card reference at all (verified over 5.026 production bons). |
| Looking for "which wristbands are currently open / still inside" | Doesn't exist. `FindAccessZones` gives a **counter** (`visitorsCurrent`), never the bands behind it. `General/FindPersonCards` is the only card endpoint; every presence/passage/scan endpoint 404s (full list in `ENDPOINTS.md`). Answer occupancy questions from the counter, identity questions from Gantner Relaxx / the ReCreateX back-office. |
