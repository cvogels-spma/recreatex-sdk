...
var ExpositionsModule = class {
   ...
   */
  async listPeriods(expositionId, fromIso, untilIso, callOpts, paginateOpts = {}) {
    const pageOptions = { pageSize: 200, ...paginateOpts };
    return paginate(
      ({ pageIndex, pageSize }) => this.listPeriodsPage(
        expositionId,
        fromIso,
        untilIso,
   ...
   */
  async listPeriodsPage(expositionId, fromIso, untilIso, paging, callOpts) {
    const data = await this.client.post(
...
var GeneralModule = class {
   ...
   */
  async findPersonCards(criteria = {}, callOpts) {
    const Criteria = {
      Paging: {
        PageIndex: criteria.pageIndex ?? 0,
        PageSize: criteria.pageSize ?? 50
